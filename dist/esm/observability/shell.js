...
export class ObservabilityShell {
    ...
    async runPublish(topic, correlationId, messageId, source, executor, extra) {
        ...
        try {
            ...
            if (this.d.features.metrics)
                this.d.metrics.incCounter(METRIC_NAMES.PUBLISH_ERRORS_TOTAL, labels);
            if (this.d.features.logging)
    ...
    async runConsume(topic, meta, executor, onResultMetric) {
        ...
        try {
            ...
            if (this.d.features.metrics)
                this.d.metrics.incCounter(METRIC_NAMES.CONSUME_ERRORS_TOTAL, labels);
            if (this.d.features.logging)
    ...
    updateConsumeResultMetrics(result, labels) {
        ...
        switch (result.status) {
            case ProcessStatus.ACK:
                this.d.metrics.incCounter(METRIC_NAMES.ACK_TOTAL, fullLabels);
                break;
            case ProcessStatus.NACK:
                this.d.metrics.incCounter(METRIC_NAMES.NACK_TOTAL, fullLabels);
                break;
            case ProcessStatus.RETRY:
                this.d.metrics.incCounter(METRIC_NAMES.RETRY_TOTAL, fullLabels);
                break;
            case ProcessStatus.DLQ:
                this.d.metrics.incCounter(METRIC_NAMES.DLQ_TOTAL, fullLabels);
                break;
        }
    }
}
